...
class DenseMLPWithLoRA(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        gate_output = self.activation_fn(torch.matmul(input, self.W_gate))
        up_output = torch.matmul(input, self.W_up)
        gated_output = gate_output * up_output
        output = torch.matmul(gated_output, self.W_down)
        ...
        if self.lora_rank > 0:
            with torch.random.fork_rng():  # not letting lora_dropout_seed interfere with the global random seed
                ...
                X_times_A_times_B = torch.matmul(input, self.A_r).matmul(self.B_r)
                lora_output = self.lora_dropout((self.lora_alpha / self.lora_rank) * X_times_A_times_B)
            output += lora_output

        return output.to(input_dtype).to(input_device)
...
class SparseMLPWithLoRA(nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        output = torch.zeros_like(input, device=self.device, dtype=self.dtype)
        ...
        for local_idx, expert in enumerate(self.local_experts):
            ...
            if mask.any():
                ...
                routed_input = input[mask]            # [num_routed_tokens, hidden_size]
        ...
        return output.to(input.dtype).to(input.device)
