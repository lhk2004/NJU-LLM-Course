...
class NTKAwareRoPE(nn.Module):
    ...
    def compute_positional_embeddings(self, seq_len: int):
        ...
        inv_freq = 1.0 / (base ** (torch.arange(0, self.dim, 2, dtype=torch.float32, device=self.device) / self.dim))
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        if seq_len > self.max_seq_len:
            ...
            if self.dynamic:
                ...
                t = torch.arange(new_max_seq_len, device=self.device, dtype=torch.float32)
                freqs = torch.outer(t, inv_freq)
                emb = torch.cat((freqs, freqs), dim=-1)
                C_ = torch.cos(emb)
                S_ = torch.sin(emb)

                return apply_rotary_pos_emb(input, C_[:input.shape[1]], S_[:input.shape[1]])
        
        # apply rotary positional embedding 
        return apply_rotary_pos_emb(input, self.cos[:input.shape[1]], self.sin[:input.shape[1]])  